...
resource "null_resource" "docker_build" {
    ...
    provisioner "local-exec" {
        command = "docker build --platform linux/amd64 -t ${local.image_tag} --file ../${var.service_name}/Dockerfile --target prod ../${var.service_name}"
    }
}
...
resource "null_resource" "docker_login" {
    ...
    provisioner "local-exec" {
        command = "echo ${var.password} | docker login ${var.login_server} --username ${var.username} --password-stdin"
    }
}
